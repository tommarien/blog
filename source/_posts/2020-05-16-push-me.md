...
updated: 2020-05-17 22:36:05
---

In the spirit of honing our skills, we'll tackle one more component. This time i've picked the [button](https://getbootstrap.com/docs/4.5/components/buttons/) component.

So let's start with the basics, a `<button />` has `children`, an `onClick` handler, a `type` (`submit`, `button` or `reset`) and can be `disabled`. One thing to keep in mind with buttons is that if you don't supply a type to a button it will default to `submit`, which is generally not what you want.
...
We just need to add that bootstrap flavor, we add `variant` into the mix:
